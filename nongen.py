...
for year in years:
    ...
    if len(ageParents[year]) > 0:
        print("%2.2f" % 1.0*sum(ageParents[year])/len(ageParents[year]),
              end=' ', file=demoout)
    else:
        print("00.00", end=' ', file=demoout)

    for age in ages:
        print("%3d" % len([x for x in years[year]["age"] if x == age]),
              end=' ', file=demoout)
        print("%3d" % len([x for x in years[year]["mage"] if x == age]),
              end=' ', file=demoMout)
        print("%3d" % len([x for x in years[year]["fage"] if x == age]),
              end=' ', file=demoFout)
    print("", file=demoout)
    print("", file=demoMout)
    print("", file=demoFout)
...
for gen in parentAgeGen:
    ...
    for age in ages:
        print("%3d" % len([x for x in parentAgeGen[gen] if x == age]),
              end=' ', file=breeout)
    print("", file=breeout)
